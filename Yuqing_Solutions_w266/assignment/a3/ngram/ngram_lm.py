...
class AddKTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        for key in self.counts.keys():
            c = 0
            for sub_key in self.counts[key].keys():
                c += self.counts[key][sub_key]
            self.context_totals[key] = c
...
class KNTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        for context in self.counts.keys():
            self.context_nnz[context] = len(self.counts[context].keys())

        # Compute type fertilities, and the sum z_tf.
        for w in self.type_contexts.keys():
    ...
    def kn_interp(self, word, context, delta, pw):
        ...
        cb = self.context_totals.get(context)
        p_kn_c = self.type_fertility.get(word , 0) / self.z_tf

        if cb is None:
            return pw
        else:
            cbc = self.counts.get(context).get(word, 0)
            if cbc - delta > 0:
                return (cbc - delta) / cb + pw * p_kn_c
            else:
                return pw * p_kn_c
