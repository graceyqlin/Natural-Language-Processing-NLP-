...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        embed = tf.reshape(tf.nn.embedding_lookup(self.W_in_, self.input_w_), 
                    [self.batch_size_, self.max_time_, self.H])

        # Construct RNN/LSTM cell and recurrent layer.
        self.cell_ = MakeFancyRNNCell(self.H, self.dropout_keep_prob_, num_layers = 1)
        self.initial_h_ = self.cell_.zero_state(self.batch_size_, dtype = tf.float32)

        self.output, self.final_h_ = tf.nn.dynamic_rnn(
                                         cell = self.cell_,
                                         inputs = embed,
        ...
        self.b_out_ = tf.Variable(tf.random_uniform([self.V], -1.0, 1.0), name = "b_out")

        self.logits_ = tf.add(matmul3d(self.output, self.W_out_), self.b_out_) ## X: [m,n,k] W: [k,l]  XW: [m,n,l]

        # Loss computation (true loss, for prediction)
        self.loss_ = tf.reduce_mean( tf.nn.sparse_softmax_cross_entropy_with_logits(    
                                    labels=tf.reshape(self.target_y_, [-1, 1] ),
                                    logits=tf.reshape(self.logits_, [-1, 1, self.V] ) )
                                    )
    ...
    def BuildTrainGraph(self):
        ...
        self.train_loss_ = tf.reduce_mean( tf.nn.sampled_softmax_loss(
                                                weights = tf.transpose(self.W_out_), ## [num_classes, dim]
                                                biases = self.b_out_,  ## [num_classes]
                                                labels = tf.reshape(self.target_y_, [-1,1]),
                                                inputs = tf.reshape(self.output, [-1, self.H]),
        ...
        self.train_step_ = opt.minimize(self.train_loss_)
    ...
    def BuildSamplerGraph(self):
        ...
        # print(pred_samples_.get_shape().as_list())
        #### END(YOUR CODE) ####
