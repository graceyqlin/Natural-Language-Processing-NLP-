...
def embedding_layer(ids_, V, embed_dim, init_scale=0.001):
    ...
    W_embed = tf.get_variable("W_embed", shape=[V, embed_dim], dtype=tf.float32, initializer = tf.random_uniform_initializer(-init_scale, init_scale))
    xs_ = tf.nn.embedding_lookup(W_embed, ids_)
...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        ...
        W_out_ = tf.get_variable("W_out", shape = [h_.shape[1], num_classes], dtype=tf.float32, initializer = tf.random_normal_initializer())
        b_out_ = tf.get_variable("b_out", shape = [ num_classes], dtype=tf.float32,
                  initializer = tf.zeros_initializer()) 
        logits_ = tf.add(tf.matmul(h_, W_out_), b_out_)  # replace with (h W + b)
        #### END(YOUR CODE) ####
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    xs_ = tf.multiply(xs_, mask_)  # [batch_size, max_len, embed_dim] 

    # Sum embeddings: [batch_size, max_len, embed_dim] -> [batch_size, embed_dim]
    h_0 = tf.reduce_sum(xs_, axis = 1)
    
    # Build a stack of fully-connected layers
    h_ = fully_connected_layers(h_0, hidden_dims, dropout_rate = dropout_rate, is_training = is_training)
