...
def affine_layer(hidden_dim, x):
    ...
    W = tf.get_variable("W", shape=[x.shape[1],hidden_dim], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
...
def fully_connected_layers(hidden_dims, x):
    ...
    # START YOUR CODE
#     h(n) = tf.nn.relu(h(n-1)*W + b)
     
    with tf.variable_scope("layername"):
        if hidden_dims == []:
            return x
        else:
            return tf.nn.relu(affine_layer(hidden_dims[-1],fully_connected_layers(hidden_dims[:-1],x)))
#             return fully_connected_layers(hidden_dims[1:],tf.nn.relu(affine_layer(hidden_dims[0],x)))
...
def MakeLogits(x_ph, hidden_dims):
    ...
    '''
    # YOUR CODE HERE
    return tf.squeeze(affine_layer(1, fully_connected_layers(hidden_dims, x_ph)),1)
#     return None
    # END YOUR CODE HERE
