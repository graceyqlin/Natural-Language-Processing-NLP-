...
#         print(sentence)
        ...
        for t in self.tagset:
            alpha[(0, t)] = self.initial(t) + self.emission(t, w0)
...
#                 α(i,t) =  p(wi | t)×  ∑t′  p(t | t′)×α(i−1,t′)
                for t in self.tagset:
                    a = []
                    for t_1 in self.tagset:
                        a.append(self.transition(t_1, t) + alpha[(i-1, t_1)])
                        
                    alpha[(i,t)] = self.emission(t, w) + logsumexp(a)
...
#         print(alpha)
    ...
    def backward(self, sentence):
        ...
        for i in range(n-2, -1, -1):
            w = sentence[i+1]
            # β(i−1,t)=∑t′p(wi | t′)×p(t′ | t)×β(i,t′)
            for t in self.tagset:
                b = []
                for t_1 in self.tagset:
                    b.append(self.emission(t_1,w) + self.transition(t,t_1) + beta[(i+1,t_1)])
                    
                beta[(i,t)] = logsumexp(b)
        # END(YOUR CODE)
    ...
    def build_viterbi_delta(self, sentence):
        ...
        # δ(0,t)=p(t)×P(x0 | t)
        for t in self.tagset:
            delta[(0, t)] = self.initial(t) + self.emission(t, sentence[0])
            bp[(0,t)] = None
        # δ(i,t)=p(xi | t)×maxt′[δ(i−1,t′)×p(t | t′)]
        for i in range(1, len(sentence)):
            for t in self.tagset:
                a = []
                for t_1 in self.tagset:   
                    a.append( delta[(i-1, t_1)] + self.transition(t_1, t) )
                delta[(i,t)] = self.emission(t, sentence[i]) + max(a)
                
                _, bp[(i,t)] = max([(delta[(i-1,t)], t) for t in self.tagset])
